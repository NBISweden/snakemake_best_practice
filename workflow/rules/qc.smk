...
rule qc_multiqc:
    ...
    input:
        unpack(qc_multiqc_input),
    params:
        config["qc_multiqc"]["options"],
    log:
...
rule qc_samtools_coverage:
    ...
    output:
        txt="results/qc/samtools/{sample}.coverage.txt",
    input:
        bam="data/interim/map/bwa/{sample}.bam",
    threads: 1
    conda:
        "../envs/samtools.yaml"
    envmodules:
        "samtools/1.12",
    log:
        "logs/qc/samtools/{sample}.coverage.log",
    shell:
...
rule qc_plot_samtools_coverage:
    ...
    output:
        png=report(
            "results/qc/samtools/{sample}.coverage.png",
...
rule qc_plot_samtools_coverage_collate:
    """Plot the meandepth coverage for multiple inputs"""
    output:
        png=report(
            "results/qc/samtools/all.coverage.png",
            caption="../report/allcoverage.rst",
            category="Coverage",
        ),
    input:
        txt=expand(
            "results/qc/samtools/{sample}.coverage.txt", sample=samples.index.tolist()
        ),
    log:
        "logs/results/qc/samtools/all.coverage.png.log",
    script:
